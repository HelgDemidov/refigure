...
Otherwise scan `docs/*.md` for specs that are BOTH: missing a `## Статус выполнения` section, AND whose declared branch (the `**Ветка:**` line) still exists — local or `origin/` — and isn't merged into `main`. The `**Ветка:**` line is what distinguishes an actual spec from a general design doc (like `docs/v1-scope-and-api-design-2026-08-04.md`), which has none. This two-signal check avoids false positives on old completed specs whose status section was never backfilled.
