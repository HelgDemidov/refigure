...
Первая стадия фазы I (`docs/project-meta/execution-sequence/execution-sequence-2026-08-04.md`, 10% трудоёмкости v1) —
...
почти-неизменных модулей из исходного пайплайна в `refigure/` как плоские
...
`docs/project-meta/v1-scope-and-api-design/v1-scope-and-api-design-2026-08-04.md`, которая включает и материал стадии 2):

| Источник (исходный пайплайн, `pipeline/scripts/convert/`) | Назначение (`refigure/`) | Строк | Зависимости после переноса |
|---|---|---|---|
...
- Вызов `zipsafe.check_archive()` — в исходном пайплайне живёт в `converters.py`/
...
Решено 2026-08-04, коммит `296da93` (прямой коммит в main, без ветки/PR —
тривиальная правка): `ruff format .` по всем 5 файлам, семантика не
изменилась (`ast.dump()` до/после идентичен на всех пяти), `ruff format
--check .` теперь зелёный.
