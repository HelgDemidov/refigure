...
- [x] 7b часть 1 (test-coverage hardening) — PR #14, combined-покрытие
  100%. Часть 2 (security-инструменты + аудит) — спек в разработке,
  гейтит релиз наравне с остальными пунктами этой стадии.
...
- [x] **Branch protection на `main`** — настроено 2026-08-06:
  ...
  `allow_deletions=false`, без обязательного PR-ревью (сохраняет
  задокументированный workflow «рутинные изменения — прямой коммит»).
  **`required_status_checks` осознанно НЕ включён** — на момент настройки
  ни один CI-контекст ни разу не отчитался успешно (продолжающийся
  GitHub Actions инцидент, см. `project_coverage_hardening`/
  `project_readme_and_demo` memory) — включение required-checks сейчас
  риск перманентно заблокировать мерж до восстановления Actions.
  Доделать: включить `required_status_checks` с реальными именами джобов
  после первого подтверждённого зелёного прогона на `main`.
...
- [x] Публичность репо + branch protection — сделано 2026-08-06, раньше
  мержа этого PR (см. §4).
