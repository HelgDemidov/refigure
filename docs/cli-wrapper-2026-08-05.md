...
Смержено 2026-08-05, PR #9 (`feat/cli-wrapper` → `main`, merge-коммит
`012b654`). Все 9 запланированных коммитов + чек-лист выше выполнены
как заявлено, без отклонений от скоупа спека.

Незапланированная находка по ходу работы: первый прогон CI (`test-unit`
job) вскрыл реальный баг конкурентности в `xlsx.convert()` — не связан с
самим CLI, но найден именно новым тестовым файлом (`test_cli.py`),
сдвинувшим порядок выполнения существующего `test_concurrency.py`.
Корень — общий module-level `lxml.etree.XMLParser()` внутри самого
`openpyxl` (не в коде refigure). Закрыто `threading.Lock()` в
`refigure/xlsx.py` (коммиты `2c02858`/`b7da553`), не тестовой заплаткой —
подробности в `project_openpyxl_concurrent_parser_fragility` memory.

Вне скоупа осталось ровно то, что и планировалось (раздел выше) — ничего
не довырезано по ходу реализации.
