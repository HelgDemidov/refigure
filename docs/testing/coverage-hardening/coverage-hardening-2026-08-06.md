...
Смержено 2026-08-06, PR #14 (`test/coverage-hardening` → `main`,
merge-коммит `51b8c49`), все 9 запланированных коммитов легли как есть.
Финальный результат превзошёл цель спека: combined `tests/unit`+
`tests/integration` — не 95%, а **100%** (1528/1528 стейтментов, 0
непокрытых, 406 passed/3 skip), подтверждено полным авторитетным
прогоном (~12 мин) непосредственно перед мержем.

По ходу реализации закрыто несколько реальных находок сверх плана §2:
xychart-beta вырожденный диапазон осей (`y_min == y_max`) в
`chart_render.py` не имел покрытия вообще; guard на `mermaidx` в
`chart_render.py` не имел выделенного теста (в отличие от остальных 3) —
добавлен перед проставлением pragma; openpyxl округляет
целочисленный `float` до `int` при записи — тест на `str(int(v))`
потребовал прямого патча XML ячейки.

Смержено в обход CI — тот же инцидент GitHub Actions, что и PR #13, но
эскалировавший дальше: на коммите мержа `51b8c49` **0 check-runs
вообще**, даже не "Queued" — веб-хук не долетел до Actions
(`gh api .../commits/51b8c49/check-runs` → `total_count: 0`),
githubstatus.com подтверждал `major_outage`/`investigating` на момент
мержа. **CI-статус `51b8c49` на `main` не подтверждён** — перепроверка
потребует запушить именно этот SHA в одноразовую ветку после
восстановления Actions (у `ci.yml` нет branch-фильтра, GitHub
привязывает чеки к SHA, а не к имени ветки).
