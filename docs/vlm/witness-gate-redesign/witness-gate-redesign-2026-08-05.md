...
- `vlm_verify=True` по умолчанию — остаётся opt-in, как и сам `use_vlm`
  (не анонсирован в v1).
- Слой 2 дерева конфигурации (§6): локальная модель / выбор облачного
  провайдера помимо OpenRouter — сегодня расширяемость только на уровне
  протокола (`VlmClient`), не готовая опция; отдельный будущий спек.
- 2-раундовый адверсариальный self-review (§4) — рассмотрен и отклонён,
  литература + собственный прецедент проекта (отказ от SelfCheckGPT)
  сходятся: риск регресса без устранения системной слепоты.

## Статус выполнения

**Смержено:** 2026-08-06, PR #11 (`f2fa952`), ветка `feat/witness-gate-redesign`.

Финальная архитектура разошлась с §2's исходным планом — self-judge
(модель судит саму себя, как спроектировано изначально) провалил
обязательную живую валидацию §5 (30%/12% recall на 24 размеченных
ответах, ложные срабатывания, нестабильность между вызовами) и был
заменён mid-PR на §6's независимую судейскую архитектуру
(`Config.vlm_judge_mode` solo/panel, панель — дефолт, UNION, 2
зафиксированных судьи). Полная разборка: `docs/vlm/vlm-model-calibration/
judge-defects-validation-2026-08-06.md`, memory
`project_witness_gate_redesign`.

Тот же PR попутно донёс `refigure/`+`tests/` реорганизацию пакета
(плоские модули → `core`/`docx`/`xlsx`/`vlm` подпакеты) и нашёл ею же
внесённый 5-й реальный production-баг (`docx_groups.py`, вложенный в
`docx/`, транзитивно требовал mammoth для `refigure.vlm` — сломанная
extras-изоляция `[vlm]`), пойманный CI-матрицей и исправленный тем же
PR (`b7ebc01`) — не отдельным follow-up.

Вне скоупа этого PR, зафиксировано выше: слой 2 дерева конфигурации
(локальная модель/выбор провайдера) — отдельный будущий спек.
