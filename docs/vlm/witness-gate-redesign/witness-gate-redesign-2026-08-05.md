...
- [x] сквозные smoke-тесты против реального `OpenRouterClient` (не фейков)
      для всех 3 реализованных веток: panel (дефолт), solo (явный),
      `vlm_verify=False` с 3 разными `vlm_model` — все прошли чисто,
      подтверждают пламбинг, не точность (она уже подтверждена §3-bis/§5
      реальными данными)
