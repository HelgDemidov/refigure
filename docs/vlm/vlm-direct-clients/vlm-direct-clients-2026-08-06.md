...
**Смержено:** 2026-08-06, PR #12 (`2bf0ba5`), ветка `feat/vlm-direct-clients`.
Все 10 запланированных коммитов реализованы как есть, без отклонений от
плана. Плюс 1 доп. коммит после мержа (`4e2cdda`, напрямую в `main`,
рутинное однокоммитное дополнение — таблица по доступности облачных
аккаунтов для §3, без нового PR).

По ходу реализации найдено и исправлено 2 реальных бага, не
предусмотренных изначальным планом: (1) `vlm-direct` extra ломался на
импорте без `[vlm]` — 3-й экземпляр extras-isolation-класса, но, в отличие
от первых двух, реальная связь, а не случайность (см. `project_extras_isolation_bug`
memory); (2) собственные live-тесты Bedrock/Vertex падали хардфейлом
вместо skip при отсутствии `boto3`/`google-auth` — гвард проверял не тот
уровень. Оба воспроизведены и подтверждены исправленными в реальных
чистых venv, не только предположены.

Живой прогон против реальных AWS/GCP/Azure аккаунтов не выполнен — среда
реализации не имеет доступа ни к одному из трёх; остаётся ручным шагом
для пользователя (см. §3's таблицу доступности аккаунтов). Полная
разборка: `project_vlm_direct_clients` memory.
