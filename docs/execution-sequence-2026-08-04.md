...
8. **Релиз — 5%.** В основном гейт-чеклист + механика паблиша (сборка
   ...
   зелёные) + 6 (CI зелёный) + 7 (README готов) + branch protection на `main`
   (no-force-push/no-deletion/required-status-checks — решение 2026-08-04,
   `CLAUDE.md` §Git workflow: до релиза не нужно, публичный OSS-статус делает
   это частью гейта, не факультативом). Не ждёт 4b — VLM изолирован
